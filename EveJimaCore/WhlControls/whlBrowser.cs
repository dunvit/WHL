...
namespace EveJimaCore.WhlControls
{
    public partial class whlBrowser : UserControl
    {
        ...
        public void DisposeTabs()
        {
            ...
            foreach (var VARIABLE in browserTabControl.Controls)
            {
        ...
        public void Dispose()
        {
            ...
            {
                ChromiumWebBrowser control = page.Controls[0] as ChromiumWebBrowser;

                if (control != null)
                {
                    Log.ErrorFormat("[whlBrowser.AddTab] Dispose CefSharp browser address {0}", control.Address);
                    control.Dispose();

                }
            }

            Log.ErrorFormat("[whlBrowser.AddTab] Cef Shutdown {0}", "");
            Cef.Shutdown();
        }
